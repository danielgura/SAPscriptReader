<?php
	...
	function highlight_row($name, $line, $row) {
		...
		if((substr($row, 2, 5) == 'ENDIF' || substr($row, 2, 7) == 'ENDCASE')
			&& substr($row, 0, 2) != '/*') { $tab_count--; }

		for($t = 0; $t < $tab_count; $t++) { $tab .= '&nbsp;&nbsp;&nbsp;'; }

		if((substr($row, 2, 2) == 'IF' || substr($row, 2, 4) == 'CASE')
			&& substr($row, 0, 2) != '/*') { $tab_count++; }
